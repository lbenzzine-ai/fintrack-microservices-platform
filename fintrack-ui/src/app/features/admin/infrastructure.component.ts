  ...
  template: `
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
      <div class="card">
        ...
        <button (click)="checkHealth()" class="btn-refresh mt-4">Refresh</button>
      </div>

      <div class="card">
        <div class="section-title">Platform links</div>
        <div class="table-row"><span class="text-sm">API Gateway</span>
          <a [href]="env.swagger"  target="_blank" class="link-gold">Swagger →</a></div>
        <div class="table-row"><span class="text-sm">Eureka</span>
          <a [href]="env.eureka"   target="_blank" class="link-gold">Registry →</a></div>
        <div class="table-row"><span class="text-sm">Grafana</span>
          <a [href]="env.grafana"  target="_blank" class="link-gold">Dashboards →</a></div>
        <div class="table-row"><span class="text-sm">Zipkin</span>
          <a [href]="env.zipkin"   target="_blank" class="link-gold">Traces →</a></div>
        <div class="table-row"><span class="text-sm">Kibana</span>
          <a [href]="env.kibana"   target="_blank" class="link-gold">Logs →</a></div>
        <div class="table-row"><span class="text-sm">Kafdrop</span>
          <a [href]="env.kafdrop"  target="_blank" class="link-gold">Topics →</a></div>
        <div class="table-row"><span class="text-sm">Mailhog</span>
          <a [href]="env.mailhog"  target="_blank" class="link-gold">Emails →</a></div>
      </div>
    </div>
...
export class InfrastructureComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private buildCharts() {
    ...
    this.charts.push(new Chart(this.requestChartRef.nativeElement, {
      type: 'line',
      data: { labels, datasets: [{ data: reqData, borderColor: gold,
        backgroundColor: 'rgba(196,163,82,0.1)', fill: true, tension: 0.4, borderWidth: 2, pointRadius: 0 }] },
    ...
    this.charts.push(new Chart(this.latencyChartRef.nativeElement, {
      type: 'bar',
      data: { labels: svcs, datasets: [{ data: latencies,
        backgroundColor: latencies.map(l => l < 100 ? 'rgba(76,175,128,0.7)' : l < 200 ? 'rgba(196,163,82,0.7)' : 'rgba(224,112,112,0.7)'),
        borderRadius: 4 }] },
      options: { responsive: true, plugins: { legend: { display: false } },
        scales: { x: { grid: { color: gridColor }, ticks: { color: textColor, font: { size: 10 } } },
                  y: { grid: { color: gridColor }, ticks: { color: textColor, font: { size: 10 } } } } }
