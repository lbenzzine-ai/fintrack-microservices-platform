...
@Component({
  ...
  template: `
    <div class="flex justify-between items-center mb-7">
      <div class="font-display text-2xl">Transaction history</div>
    ...
    <div class="grid grid-cols-3 gap-5 mb-5">
      ...
      <div class="card">
        <div class="section-title">Fee collected (total)</div>
        <div class="flex flex-col justify-center h-32">
          ...
          <div class="mt-4 text-xs text-slate-muted">Avg fee per tx</div>
    ...
    <!-- Table -->
    <div class="card">
      <div class="flex justify-between items-center pb-3 border-b border-white/5 mb-1">
        <span class="text-xs text-slate-muted uppercase tracking-widest" style="flex:2">Description</span>
        <span class="text-xs text-slate-muted uppercase tracking-widest" style="flex:1">Amount</span>
        <span class="text-xs text-slate-muted uppercase tracking-widest" style="flex:1">Fee</span>
        <span class="text-xs text-slate-muted uppercase tracking-widest" style="flex:1">Risk</span>
        <span class="text-xs text-slate-muted uppercase tracking-widest" style="flex:1">Status</span>
        <span class="text-xs text-slate-muted uppercase tracking-widest" style="flex:1">Date</span>
      </div>
...
export class HistoryComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  load() {
    this.loading = true;
    this.txService.getMyTransactions(this.page, this.pageSize).subscribe({
      next: res => {
  ...
  private buildCharts() {
    ...
    const compensated = this.transactions.filter(t => t.status === 'COMPENSATED').length || 1;
    ...
    const statusChart = new Chart(this.statusChartRef.nativeElement, {
      ...
      data: {
        ...
        datasets: [{
          data: [completed, failed, initiated, compensated],
          backgroundColor: [
            'rgba(76,175,128,0.8)',
            'rgba(224,112,112,0.8)',
            'rgba(196,163,82,0.8)',
            'rgba(138,155,181,0.8)'
          ],
          borderColor: navy,
          borderWidth: 2
        }]
    ...
    const amountChart = new Chart(this.amountChartRef.nativeElement, {
      type: 'bar',
      data: {
        labels: buckets,
        datasets: [{
          data: counts,
          backgroundColor: 'rgba(196,163,82,0.6)',
          borderColor: '#C4A352',
          borderWidth: 1,
          borderRadius: 4
        }]
