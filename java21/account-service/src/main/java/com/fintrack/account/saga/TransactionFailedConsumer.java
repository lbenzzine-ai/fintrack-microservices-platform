...
public class TransactionFailedConsumer {
    ...
    public void onTransactionFailed(@Payload TransactionFailedEvent event) {
        if (!event.alreadyDebited()) {
            log.debug("[SAGA] tx-failed tx={} not yet debited — no compensation needed", event.transactionUuid());
            return;
        }
        log.warn("[SAGA] compensating tx={} account={} reason={}",
                event.transactionUuid(), event.fromAccountUuid(), event.reasonCode());
        accountService.compensateCredit(event.fromAccountUuid(), event.amount(), event.fee(), event.reasonCode());
    }
}
