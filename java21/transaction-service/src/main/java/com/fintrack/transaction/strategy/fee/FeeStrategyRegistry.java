 ...
 * they support. The sealed interface means any new Strategy must be added to the {@code permits}
 * clause too — defending against accidental "extra" Strategies that bypass the registry contract.
 */
@Slf4j
@Component
public class FeeStrategyRegistry {
    ...
    public FeeStrategyRegistry(List<FeeStrategy> strategies) {
        this.byName = strategies.stream()
                .collect(Collectors.toUnmodifiableMap(FeeStrategy::name, s -> s));
