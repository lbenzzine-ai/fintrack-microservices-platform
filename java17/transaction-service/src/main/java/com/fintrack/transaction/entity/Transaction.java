...
/**
 * Transaction aggregate — the saga subject. Owns its own status machine: PENDING → INITIATED →
 * DEBITED → COMPLETED, with FAILED/COMPENSATED branches written by the saga consumers.
 *
 * <p>{@code READ_WRITE} L2 cache region: rows are mutable (status transitions) but immutable once
 * COMPLETED/FAILED — most reads are post-terminal, so caching pays off.
 */
@Entity
