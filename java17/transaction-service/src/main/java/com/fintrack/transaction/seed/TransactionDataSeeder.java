...
/**
 * Optional bulk seeder — generates 100k+ realistic transactions for load testing the saga,
 * fee strategies and reporting endpoints. Toggle via {@code fintrack.seed.enabled=true}.
 *
 * <p>Skipped automatically if the table already has rows so re-deploys remain idempotent.
 */
@Slf4j
@Component
...
public class TransactionDataSeeder {
    ...
    public void seed() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            String to   = (type == TransactionType.ATM_WITHDRAWAL || type == TransactionType.BILL_PAYMENT)
                    ? null
